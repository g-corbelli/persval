...
#' dimensions related to Schwartz's theoretical framework based on the 21 items
#' of the PVQ-21 European Social Survey Value Scale (Schwartz, 2003).
...
#'
#' @param df A Data frame containing the raw responses for the 21 PVQ items. If "items" is not provided, it must have exactly 21 columns, and their order must correspond to the PVQ-21 items.
#' @param items An optional vector containing the names or indices of the columns that correspond to the PVQ-21 items. Must be exactly 21 items. If NULL, the function assumes the items are the only columns given in the "df" parameter.
...
pvq21 <- function(df, items = NULL, compute = "all", correction = TRUE, na.rm = TRUE) {
  if (!is.null(items)) {
    if (length(items) != 21) {
      stop("Exactly 21 ordered items must be provided.")
    }
    if (is.numeric(items)) {
    ...
    if (ncol(df) != 21) {
      stop("The data frame must contain exactly 21 columns.")
    }
  ...
  val_cols <- list(
    c(required_columns[7], required_columns[16]),
    c(required_columns[20], required_columns[9]),
    c(required_columns[12], required_columns[18]),
    c(required_columns[3], required_columns[19], required_columns[8]),
    c(required_columns[1], required_columns[11]),
    c(required_columns[15], required_columns[6]),
    c(required_columns[21], required_columns[10]),
    c(required_columns[4], required_columns[13]),
    c(required_columns[2], required_columns[17]),
    c(required_columns[5], required_columns[14])
  )
